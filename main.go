...
func main() {
	...
	// 所有旗標 (--windows / -i / -o) 的註冊都由 commands.MapCommand 的 schema 驅動。
	vals := commands.BindFlags(flag.CommandLine)
	flag.Usage = usage
	flag.Parse()

	// --windows：開啟視窗模式，其餘一律走命令列模式。
	if vals.Bool("windows") {
		runGUI()
		return
	}
	runCLI(vals)
...
func runCLI(vals commands.BoundFlags) {
	...
	res, err := services.BuildMap(vals.String("i"), vals.String("o"))
	if err != nil {
		fmt.Fprintln(os.Stderr, "錯誤:", err)
		os.Exit(1)
	}
	fmt.Printf("✓ 掃描 %d 個套件、%d 個 .go 檔、%d 個 @ai 函式節點，已輸出 %d 篇筆記到 %s\n",
		res.Packages, res.GoFiles, res.Functions, res.Notes, res.OutputDir)
}
